        ...
        <?php
            if (mysqli_num_rows($querJobs) > 1){
                while ($rows = mysqli_fetch_assoc($querJobs)){
                    ...
                    <div class=\"p-2  border border-dark bd-highlight fs-5\">".$_SESSION["first_name"]." ".$_SESSION["last_name"]."</div>
                    <div class=\"p-2  border border-dark bd-highlight fs-5\"> ₱".$rows['salary']."</div>
                </div>";
                };
            } else {
